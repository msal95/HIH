...
export default function FormsGenerated() {
  ...
  useEffect(() => {
    ...
    getIntegrationFoms.data,
    getIntegrationFoms.isFetched,
    getIntegrationFoms.isFetching,
  ]);
  ...
  const handleChangeIntegration = (integration_id) => {
    ...
    const oneIntegration = integrationWithForms?.filter(
      (item) => item?.id === integrat_id
    );
      ...
      <Row>
        <Col sm="12">
          <Card title="Striped" className="p-3">
            <CardBody>
              <CardText>Integration Form Generated</CardText>
              <Row className="mb-3">
                {submitting && (
                ...
                <Col className="mb-1" xl="6" md="6" sm="12">
                  ...
                  {hasForm && (
                    <p className="text-danger">
                      Forms Created For This Integration
                    </p>
                  )}
                  {hasEventsError && (
                    <p className="text-danger">
                      No Event Available Of This Integration
                    </p>
                  )}
                </Col>
                {hasEvents && !hasForm && buttonHide && (
                  <Col className="mb-1 mt-2" xl="6" md="6" sm="12">
                    <Button.Ripple
                      color="primary"
                      onClick={handleGeneratedForms}
                      disabled={submitting}
                    >
                      Generate Forms
                    </Button.Ripple>
