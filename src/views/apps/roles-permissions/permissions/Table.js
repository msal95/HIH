...
import { createPermission } from "../../../../../api/rolesPermissions/apiMethods";
...
const Table = (props) => {
  ...
  const handleCreatePermissions = async (values) => {
    setIsLoader(true);
    try {
      const projectData = {
        name: values.permissionName,
        corePermission: isChecked,
        // user_id: 1,
      };
      await createPermission(projectData).then((res) => {
        if (res.status === 200) {
          // refetch();
  ...
  const renderForm = () => {
    if (store.selected === null) {
      return (
        <Row tag={Form} onSubmit={handleSubmit(handleCreatePermissions)}>
          <Col xs={12}>
