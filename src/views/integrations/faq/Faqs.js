    ...
    if (eventFormDelete.isSuccess) {
      ...
      if (response === 200) {
        ...
        const filteredResources = dataVAlue?.resources?.map((resource) => ({
          ...resource,
          events: resource.events.filter(
            (event) => event.id !== resourceIdToRemove
          ),
        }));
        const eventsData = dataVAlue?.events.filter(
          (event) => event.id !== resourceIdToRemove
        );
  ...
  const handleForms = (event) => {
    ...
    if (filter === "Events With forms") {
      const filteredResources = dataFull?.resources?.map((resource) => ({
        ...resource,
        events: resource.events.filter(
          (item) => item.bpmn_form !== null && item.hasOwnProperty("bpmn_form")
        ),
      }));
      const eventsData = dataFull?.events.filter(
        (item) => item.bpmn_form !== null && item.hasOwnProperty("bpmn_form")
      );
      setDataValue((prevState) => ({
        ...prevState,
                              ...
                              <div>
                                <UncontrolledDropdown>
                                  ...
                                  <DropdownMenu>
                                    ...
                                    >
                                      <Icon.Edit className="me-50" size={15} />{" "}
                                    ...
                                    >
                                      <Icon.Trash className="me-50" size={15} />{" "}
                                      <span className="align-middle">
                                        Delete
                                      </span>
                                    </DropdownItem>
